...
class CircleStatus(str, enum.Enum):
    ...
    COMPLETED = "completed"
    ARCHIVED = "archived"
    # Legacy alias for Person 2/3 code that may use "forming"/"defaulted"
    forming = "forming"
    defaulted = "defaulted"
...
class User(Base):
    ...
    role: Mapped[UserRole] = mapped_column(
        SQLEnum(UserRole), default=UserRole.MEMBER, server_default=UserRole.MEMBER.name, nullable=False
    )
    current_trust_score: Mapped[Decimal] = mapped_column(
        Numeric(5, 2), default=Decimal("100.00"), server_default="100.00", nullable=False
    )
    is_verified: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false", nullable=False)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, server_default="true", nullable=False)
    # Person 3 fields (USSD/IVR/WhatsApp support)
    ...
    is_facilitator: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false", nullable=False)
    ...
    __table_args__ = (
        CheckConstraint(
            "current_trust_score >= 0.00 AND current_trust_score <= 100.00",
            name="chk_user_trust_score_bounds"
        ),
    )
    ...
    contributions: Mapped[list["Contribution"]] = relationship(
        "Contribution", back_populates="user", lazy="select"
    )
    payouts: Mapped[list["Payout"]] = relationship(
        "Payout", back_populates="receiver", lazy="select"
    )
    ...
    transactions_received: Mapped[list["Transaction"]] = relationship(
        "Transaction", foreign_keys="[Transaction.receiver_id]", back_populates="receiver", lazy="select"
    )
    trust_score_history: Mapped[list["TrustScore"]] = relationship(
        "TrustScore", back_populates="user", cascade="all, delete-orphan", lazy="select"
    )
    consents: Mapped[list["ConsentRecord"]] = relationship(
        "ConsentRecord", back_populates="user", lazy="select"
    )
...
class Circle(Base):
    ...
    current_member_count: Mapped[int] = mapped_column(default=0, server_default="0", nullable=False)
    current_round: Mapped[int] = mapped_column(default=1, server_default="1", nullable=False)
    payout_order_type: Mapped[str] = mapped_column(
        String(50), default="random", server_default="random", nullable=False
    )
    status: Mapped[CircleStatus] = mapped_column(
        SQLEnum(CircleStatus), default=CircleStatus.PENDING, server_default=CircleStatus.PENDING.name, nullable=False
    )
    created_by: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="RESTRICT"), nullable=False)
    # Person 2 fields
    pool_balance: Mapped[float] = mapped_column(Float, default=0.0, server_default="0.0", nullable=False)
    ...
    __table_args__ = (
        ...
        CheckConstraint("current_round >= 1", name="chk_circle_current_round_min"),
    )

    creator: Mapped["User"] = relationship(
        "User", foreign_keys=[created_by], back_populates="created_circles", lazy="select"
    )
    memberships: Mapped[list["Membership"]] = relationship(
        "Membership", back_populates="circle", cascade="all, delete-orphan", lazy="select"
    )
    contributions: Mapped[list["Contribution"]] = relationship(
        "Contribution", back_populates="circle", cascade="all, delete-orphan", lazy="select"
    )
    payouts: Mapped[list["Payout"]] = relationship(
        "Payout", back_populates="circle", cascade="all, delete-orphan", lazy="select"
    )
    transactions: Mapped[list["Transaction"]] = relationship(
        "Transaction", back_populates="circle", lazy="select"
    )
...
class Membership(Base):
    ...
    status: Mapped[MembershipStatus] = mapped_column(
        ...
        server_default=MembershipStatus.PENDING.name,
        nullable=False
    )
    position_in_payout_queue: Mapped[Optional[int]] = mapped_column(nullable=True)
    # Person 2 compat field
    payout_order: Mapped[Optional[int]] = mapped_column(nullable=True)
    received_payout: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false", nullable=False)

    __table_args__ = (
        UniqueConstraint("user_id", "circle_id", name="uq_user_circle_membership"),
        Index("idx_membership_user_id", "user_id"),
        Index("idx_membership_circle_id", "circle_id"),
    )
...
class Contribution(Base):
    ...
    status: Mapped[ContributionStatus] = mapped_column(
        SQLEnum(ContributionStatus),
        default=ContributionStatus.PENDING,
        server_default=ContributionStatus.PENDING.name,
        nullable=False
    )
    created_at: Mapped[datetime] = mapped_column(server_default=func.now(), nullable=False)

    __table_args__ = (
        CheckConstraint("amount > 0", name="chk_contribution_amount_positive"),
        Index("idx_contribution_status", "status"),
        Index("idx_contribution_due_date", "due_date"),
        Index("idx_contribution_user_circle", "user_id", "circle_id"),
    )
...
class Payout(Base):
    ...
    status: Mapped[PayoutStatus] = mapped_column(
        ...
        server_default=PayoutStatus.SCHEDULED.name,
        nullable=False
    )
    created_at: Mapped[datetime] = mapped_column(server_default=func.now(), nullable=False)

    __table_args__ = (
        UniqueConstraint("circle_id", "payout_round", name="uq_circle_payout_round"),
        CheckConstraint("amount > 0", name="chk_payout_amount_positive"),
        Index("idx_payout_circle_round", "circle_id", "payout_round"),
        Index("idx_payout_receiver_id", "receiver_id"),
    )
...
class Transaction(Base):
    ...
    # Person 2 compat fields (circle ledger)
    ...
    __table_args__ = (
        ...
        Index("idx_transaction_sender_id", "sender_id"),
        Index("idx_transaction_receiver_id", "receiver_id"),
        Index("idx_transaction_status", "status"),
    )

    sender: Mapped[Optional["User"]] = relationship(
        "User", foreign_keys=[sender_id], back_populates="transactions_sent", lazy="select"
    )
    receiver: Mapped[Optional["User"]] = relationship(
        "User", foreign_keys=[receiver_id], back_populates="transactions_received", lazy="select"
    )
    circle: Mapped[Optional["Circle"]] = relationship(
        "Circle", back_populates="transactions", lazy="select"
    )
...
class TrustScore(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "previous_score >= 0.00 AND previous_score <= 100.00",
            name="chk_trust_score_prev_bounds"
        ),
        CheckConstraint(
            "new_score >= 0.00 AND new_score <= 100.00",
            name="chk_trust_score_new_bounds"
        ),
        Index("idx_trust_score_user_created", "user_id", "created_at"),
    )
...
# =====================================================================
# Person 3 Models (consent, partners, fairness)
# =====================================================================
...
class PartnerWebhook(Base):
    ...
    api_calls: Mapped[int] = mapped_column(Integer, default=0, server_default="0", nullable=False)
    revenue_paise: Mapped[int] = mapped_column(Integer, default=0, server_default="0", nullable=False)
    created_at: Mapped[datetime] = mapped_column(server_default=func.now(), nullable=False)
